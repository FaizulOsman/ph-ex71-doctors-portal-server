...
async function run() {
  try {
    ...
    //   Get/Read (AppointmentOptions)
    app.get("/appointmentOptions", async (req, res) => {
      const date = req.query.date;
      const query = {};
      const options = await appointmentOptionCollection.find(query).toArray();
      // Get booking for specific date (provided date)
      const bookingQuery = { appointmentDate: date };
      const alreadyBooked = await bookingsCollection
        .find(bookingQuery)
        .toArray();

      // Get all bookings by its name
    ...
    // Get Special item (Only name from an array)
    ...
    app.post("/bookings", async (req, res) => {
      ...
      // I user have already 1 booking, kick him
    ...
    // Create user in MongoDB for setup users information
